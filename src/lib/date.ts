...
export const isNewRange = (oldRange: PeriodFilter['range'], newRange: PeriodFilter['range']) => {
  const newFrom = toLocaleDate(newRange[0])!;
  const newTo = toLocaleDate(newRange[1])!;

  const [oldFrom, oldTo] = oldRange;

  return newFrom.getTime() !== oldFrom?.getTime() || newTo.getTime() !== oldTo?.getTime();
};
