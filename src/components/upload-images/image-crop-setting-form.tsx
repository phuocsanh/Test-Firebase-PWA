...
import { FormCombobox, FormDescription, FormField, FormLabel } from '../ui/form';
...
export const ImageCropSettingForm = ({ sizes = defaultSizes }: IImageCrop) => {
  ...
  const methods = useFormContext<ImageCropSetting>();
  const {
    control,
    setValue,
    // register,
      ...
      <div id="image-uploader-setting-form" className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-start gap-1">
            ...
            />
            <Label>{t('useSizeOption')}</Label>
          </div>
          <FormDescription>{t('sizeOptionActive')}</FormDescription>
        </div>
        <div className="flex items-center justify-start gap-1">
          <FormField name="Width">
            <InputNumber
              disabled={!useSizeOption}
              id="image-crop-width"
              placeholder={`${enterLabel} ${t('width')}`}
        ...
        <div className="flex items-center justify-start gap-1">
          <FormField name="Height">
            <InputNumber
              ...
              placeholder={`${enterLabel} ${t('height')}`}
              className="max-w-xs lowercase"
        ...
        <div className="mt-4 flex items-center justify-start">
          <FormLabel className="w-20">{t('zoom')}</FormLabel>
          <Controller
            name="zoom"
            control={control}
            render={({ field }) => (
        ...
        <div className="flex items-center justify-start gap-1">
          <FormLabel className="w-20">{t('rotate')}</FormLabel>
          <Controller
            name="rotation"
            control={control}
            render={({ field }) => {
