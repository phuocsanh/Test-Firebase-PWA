...
import { uploadMulti } from '@/services/common';
import { UploadPayloadType } from '@/types';
...
export const UploadImages = forwardRef<UploadImageRef, UploadImagesProps>(
  ({ className, folder, mode = 'multiple', sizes, onUploadSuccess, onRemove, ...props }, ref) => {
    ...
    const { mutateAsync } = useMutate({
      ...
      mutationFn: (payload: UploadPayloadType) => {
        const { file, folder, type } = payload;
        const formData = new FormData();
        formData.append('file', file);
        formData.append('folder', folder);
        formData.append('type', type);
        return uploadMulti(formData);
      },
    });
    ...
    const uploadImage = async (
      ...
      return Object.assign(imageFile, {
        url: `${data.host}${data.src}`,
        preview: `${data.host}${data.src}`,
      });
    };
    ...
    const { getRootProps, getInputProps, isDragActive, isDragAccept, isDragReject } = useDropzone({
      ...
      onDrop: (acceptedFiles: File[]) => {
        ...
        if (acceptedFiles.length === 1) {
          ...
          const file = createImageFile(acceptedFiles[0]);
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col items-center">
              <Button
                variant={'ghost'}
                className="hover:bg-primary-50/80 rounded-full p-2 text-primary-300 hover:text-primary-500"
              >
                <UploadCloud />
              </Button>
              {isDragAccept && (
                <p className="text-xs text-muted-foreground">{t('acceptAllFiles')}</p>
              )}
              {isDragReject && (
                <p className="text-xs text-muted-foreground">{t('acceptSpecificFiles')}</p>
              )}
              {!isDragActive && (
